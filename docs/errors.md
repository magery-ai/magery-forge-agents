...
## `DOMAIN_NOT_FOUND` / `DOMAIN_NOT_ACTIVE_FOR_CHECK` — won't fix itself with a retry

- **Where:** `POST /audits`
- **Status:** `400 Bad Request` (not 404 — this is `POST /audits` rejecting
  the request body, not a lookup on an owned resource)

`DOMAIN_NOT_FOUND` means the `domainId` you sent doesn't exist or doesn't
belong to this account — check it against `GET /domains` rather than
retrying. `DOMAIN_NOT_ACTIVE_FOR_CHECK` means the domain exists but is
currently switched off for checks on the Forge site; that's a setting your
human needs to flip, not something an audit request can work around.
